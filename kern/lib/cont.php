...
class content{
    static protected $kind_content    = array(
        ...
        //--------------------------------------------------分割线       
                        con_content	=> 2001,    //商品名称
                        con_renum       => 2002,    //回复数量
                        con_recontent   => 2003,    //回复内容
                        con_booknum     => 2004,    //预定数量
                        con_book        => 2005,    //预定
        
                        con_price       => 2010,    //价格
                        con_deteil      => 2011,    //产品细节
                        con_pic         => 2012,    //图片
                        con_clocal      => 2013,    //地点
        
        
                        con_content_hide=> 2101,     //已过期商品名称
                        con_content_myhide=> 2201,     //被和谐的商品名称
                        
        //--------------------------------------------------分割线
                        local_country   => 5001,     //国名
                        local_bundes    => 5002,     //州名
                        local_city      => 5003,      //地名
    ...
    static function select($kind_con, $kind, $page=0, $pagesize=NULL, $where=null, $orderkind=null, $deasc=null){
        if(!$kind){
            $kind=array('con_content','con_pic','con_audi');
        }
        if(!$page){
            $page=0;
        }
...
//        $kind_con=self::$kind_content[$kind_con];
        ...
        if(is_array($result)){
            ...
            foreach ($result as $r){
                ...
                $where_follow=  "(".sql_use_f::$data_columns[upid]."=".$upid." and (".$wherekind1."))";
...
//        $kind=  self::$kind_content[$kind];
    ...
    //content::change_kind(内容的id号, 类别名称);       修改类型
    static function change_kind($conid,$kind){
        return self::change($conid,$kind,sql_use_f::$data_columns[kind]);
    }
